...
function addGrowl($body,$title=false,$type=Growl::TYPE_GROWL){
    \Yii::$app->session->addFlash("growl",['type' => $type, 'title' => $title, 'body' => $body]);
}
 ...
 */
function sendMessageEmail($sendTo, $name, $subject = null, $confirmLink)
{
    try {
        $subjectStr = !is_null($subject) ? (Yii::$app->name . "|" . $subject) : 'ППТК';

        $message  = \Yii::$app->mailer->compose( 'm-template', [
            'name' => $name,
