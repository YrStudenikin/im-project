...
class ProductReadRepository extends BaseObject
{
    ...
    {
        return Product::find()->active()->new()->with('photo')->andWhere(['>', 'price_new', 0])->limit(12)->orderBy(['id' => SORT_ASC]);
    }

    public function getAllSaleForCarusel(): ActiveQuery
    {
        return Product::find()->active()->sale()->with('photo')->andWhere(['>', 'price_new', 0])->limit(12)->orderBy('id');
    }

    public function getProductByCategoryForSimilarWidget($categoryId): ActiveQuery
    {
        return Product::find()
            ->active()->with('photo')
    ...
    public function getAllByPriceCategory(Category $category): DataProviderInterface
    {
        $query = Product::find()->alias('p')->active('p')->with('photo', 'category');
        $ids = ArrayHelper::merge([$category->id], $category->getDescendants()->select('id')->column());
        $query->andWhere(['or', ['p.category_id' => $ids]]);
    ...
    {
        return Product::find()->active()->top()->with('photo')->andWhere(['>', 'price_new', 0])->limit(8)->orderBy('name');
    }

    public function getSimilarProduct()
    {
        return Product::find()->active()->top()->with('photo')->andWhere(['>', 'price_new', 0])->limit(3)->orderBy('name');
    }
}