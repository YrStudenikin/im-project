...
class ContentManageService
{
    ...
    {
        $oferta = new OfertaContentForm();
        try {
            $oferta->oferta_text = Config::getValue(Config::OFERTA_TEXT);
        } catch (ForbiddenHttpException $e) {
        }
        return $oferta;
    }

    public function updateOferta(OfertaContentForm $oferta)
    {
        Config::setValue(Config::OFERTA_TEXT, $oferta->oferta_text);
    }
}