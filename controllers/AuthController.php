...
//                            return $this->redirect('/admin');Чтобы подтвердить регистрацию перейдите по ссылке
                        ...
                        addGrowl( "Ваш аккаунт не подтвержден! <br /> Проверьте указаную Вами при регистрации эл. почту, чтобы подтвердить!", 'Ошибка!', Growl::TYPE_DANGER);
                        return $this->refresh();
                    }
                } else {
    ...
    public function actionSignUp(){
        ...
        if($authForm->load(\Yii::$app->request->post()) && $authForm->validate()) {
            ...
            if($this->confirmEmail($user->id)){
                return $this->render('email-confirm', ['email' => $user->email]);
            }
    ...
    public function confirmEmail($userId): bool
    {
        ...
        $confirmLink = Yii::$app->urlManager->createAbsoluteUrl(['/auth/confirm', 'id' => $user->id, 'key' => $user->auth_key]);

        $subject = 'Регистрация на сайте "' . Config::getValue(Config::MAIN_TITLE).'"';

        return sendMessageEmail($user->email, $user->f_name, $subject, $confirmLink);
    }
    ...
    public function actionRecover(){
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate())
        {
            $user = User::findOne(['email' => $model->email]);
            if(is_null($user)){
                addGrowl('Пользователь с такой эл. почтой не существует!', 'Ошибка',Growl::TYPE_DANGER);
                return $this->refresh();
            }
            if ( $user->sendEmailPasswordReset() ) {
                addGrowl('На указанную Вами эл. почту было выслано письмо для изменения пароля.', false, Growl::TYPE_SUCCESS);
                return $this->goHome();
            } else {
    ...
    public function actionPasswordReset($token)
    {
        ...
        if ( $model->load(Yii::$app->request->post()) && $model->validate() && $model->resetPassword($user) ) {
            addGrowl( 'Пароль изменен!', false, Growl::TYPE_SUCCESS);
            return $this->redirect(['sign-in']);
        }
    ...
    public function actionConfirm($id, $key)
    {
        ...
        if(!empty($user))
        {
            ...
            $user->save(false);
            addGrowl( 'Регистрация подтверждена! Войдите в личный кабинет', false, Growl::TYPE_SUCCESS);
            return $this->redirect('sign-in');
        }
        else{
            addGrowl('Не удалось подтвердить регистрацию. Возможно Ваш аккаунт уже подтвержден.', 'Ошибка', Growl::TYPE_DANGER);
            return $this->goHome();
        }
    }
